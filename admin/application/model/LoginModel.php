...
class LoginModel
{
     ...
     */
    public static function iniciarSesion($usuario, $contrasena, $set_remember_me_cookie = null)
    {
        ...
        $result = self::validarObtenerUsuario($usuario, $contrasena);
        ...
        self::actualizarUltimaVisitaUsuario($result->usuario);
        ...
        // successfully logged in, so we write all necessary data into the session and set "usuarioInicioSesionAdmin" to true
        self::agregarInicioSesionExitosoEnSesion($result->idUsuario, $result->idComercio, $result->perfil, $result->nombre, $result->correo, $result->iniciales);
     ...
     * @return bool|mixed
     */
    private static function validarObtenerUsuario($usuario, $contrasena)
    {
		...
		$result = UsuariosModel::obtenerUsuarioPorUsuario($usuario);
        ...
        self::resetearContadorUsuarioNoEncontrado();
     ...
     */
    private static function resetearContadorUsuarioNoEncontrado()
    {
     ...
     */
    private static function incrementarUsuarioNoEncontrado()
    {
    ...
    public static function cerrarSesion()
    {
        ...
        // self::deleteCookie($idUsuario);

        Session::destroy();
        Session::actualizarSessionId($idUsuario);
    }
     ...
     */
    public static function agregarInicioSesionExitosoEnSesion($idUsuario, $idComercio, $perfil, $nombre, $correo, $iniciales)
    {
        Session::init();
		...
		Session::set('idComercio', $idComercio);
        Session::set('perfilUsuario', $perfil);
        Session::set('nombreUsuario', $nombre);
        Session::set('correoUsuario', $correo);
        Session::set('inicialesUsuario', $iniciales);
		

        // get and set avatars
       	// Session::set('user_avatar_file', AvatarModel::getPublicUserAvatarFilePathByUserId($idUsuario));
        // Session::set('user_gravatar_image_url', AvatarModel::getGravatarLinkByEmail($correo));

        // finally, set user as logged-in
        Session::set('usuarioInicioSesionAdmin', true);

        // update session id in database
        Session::actualizarSessionId($idUsuario, session_id());
     ...
     * @param $user_name
     */
    public static function actualizarUltimaVisitaUsuario($idUsuario)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "UPDATE usuarios
					SET ultimaVisita = NOW()
    ...
    {
        return Session::usuarioInicioSesion();
    }
}

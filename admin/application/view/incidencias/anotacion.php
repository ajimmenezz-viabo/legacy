<div class="element-wrapper">
	...
	<div class="element-box-tp">
		<form name="agregarAnotacionIncidencia" id="agregarAnotacionIncidencia" novalidate method="post" class="needs-validation form form-horizontal">
			<div class="row">
				<div class="col-10 offset-md-1">
					<div class="form-group">
						<label for="anotacion">Anotación *</label>
						<div class="input-group">
							<textarea name="anotacion" id="anotacion" class="form-control" autocomplete="off" rows="3"></textarea>
						</div>
					</div>
				</div>
			</div>
			<div class="row">
				<div class="col-10 offset-md-1">
					...
					<button class="btn btn-primary btn-block btn-md"><i class="os-icon os-icon-message-square"></i><span>Agregar Anotación</span></button>
				</div>
			</div>
		</form>
	</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#agregarAnotacionIncidencia").on("submit", (function(e) {
	
			e.preventDefault();
			
			var form = document.getElementById('agregarAnotacionIncidencia');
			...
			if (form.checkValidity() === false) {
				...
				$.ajax({
					url: url + "incidencias/agregarAnotacionIncidencia",
					type: "POST",
					dataType: "JSON",
					data:  formData,
					contentType: false,
					...
					success: function(result)
					{
						if (result.success) {
							...
							$toast = toastr["success"]("Anotación agregada con éxito");
